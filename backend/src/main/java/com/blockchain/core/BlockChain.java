...
import com.blockchain.core.Transaction.Output;
...
*/
    ...
    public UTXOPool getMaxHeightUTXOPool() {
    	...
    	BlockNode node = getMaxHeightBlockNode();

    	while (node != null) {
    		for (UTXO u : node.myUtxoPool.getAllUTXO()) {
    			Boolean spent = doubleSpendingOffMap.get(u);
    			if (spent == null || !spent)
    				pool.addUTXO(new UTXO(u.getTxHash(), u.getIndex()), node.myUtxoPool.getTxOutput(u));
    		}

    		if (node.myBlock.getPrevBlockHash() != null)
    			node = map.get(new ByteArrayWrapper(node.myBlock.getPrevBlockHash()));
    		else
    			node = null;
    	}
    ...
    public boolean addBlock(Block block) {
    	...
    	for (Transaction t : newNodeTrans) {
    		for (Transaction.Input in : t.getInputs())
    			doubleSpendingOffMap.put(new UTXO(in.prevTxHash, in.outputIndex), true);
    ...
    public ArrayList<Block> getAllBlocks() {
    	...
    	for (BlockNode node : map.values())
    		blocks.add(node.myBlock);
    	return blocks;
    }

    public Block getBlockByHash(byte[] hash) {
    	BlockNode node = map.get(new ByteArrayWrapper(hash));
    	return node == null ? null : node.myBlock;
    }
    ...
    public HashMap<UTXO, Boolean> getDoubleSpendingMap() {
    	return doubleSpendingOffMap;
    }
    ...
    private BlockNode getMaxHeightBlockNode() {
    	Block best = getMaxHeightBlock();
    	return map.get(new ByteArrayWrapper(best.getHash()));
    }
}
